...
「みどぴがシャンプーを1000円買った」
「みどぴが食費を2000円払った」
などの場合は、personを必ず「みどり」としてください。

「みどぴ」という文字をGoogleスプレッドシートのpersonとして使用せず、
必ず「みどり」に変換してください。
「とも」も同様に「友介」に変換してください。
...
家計簿の取引をキャンセルする場合のみ、以下のルールに従ってください。

ユーザーが、直前に家計簿へ登録した「支出・取引そのもの」を取り消したい意図を明確に示している場合だけ、以下の形式で出力してください。

<EXPENSE> { "action": "cancel_last" } </EXPENSE>
...
重要：
「キャンセル」「取り消し」という単語が含まれているだけでは、家計簿取引のキャンセルとは判断しないでください。

ユーザーの発言が「直前に家計簿へ登録した取引を取り消したい」という文脈であることを確認できた場合のみ、cancel_lastを実行してください。

判断できない場合は、cancel_lastを実行せず、ユーザーに確認してください。
例えば、
「家計簿の直前の登録を取り消しますか？」
と確認してください。
"""
